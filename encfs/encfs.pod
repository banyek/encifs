...
EnciFS is free software; you can distribute it and/or modify it under the terms
...
encifs - mounts or creates an encrypted virtual filesystem

=head1 SYNOPSIS

B<encifs> [B<--version>] [B<-v>|B<--verbose>] [B<-c>|B<--config>] [B<-t>|B<--syslogtag>] 
...
B<EnciFS> creates a virtual encrypted filesystem which stores encrypted data in
...
If B<EnciFS> is unable to find a supported filesystem at the specified
...
allowing some control over the algorithms to use.  As B<EnciFS> matures, there
...
Shows B<EnciFS> version.  Using B<--verbose> before B<--version> may display
additional information.

=item B<-c>, B<--config>

Causes B<EnciFS> to use the supplied file as the configuration file.

=item B<-v>, B<--verbose>

Causes B<EnciFS> to enable logging of various debug channels within B<EnciFS>.
...
logged via syslog. By default the syslog tag is set to B<encifs>.

=item B<-s>

The B<-s> (I<single threaded>) option causes B<EnciFS> to run in single threaded
mode.  By default, B<EnciFS> runs in multi-threaded mode.  This option is used
during B<EnciFS> development in order to simplify debugging and allow it to run
under memory checking tools.

=item B<-f>

The B<-f> (I<foreground>) option causes B<EnciFS> to run in the foreground.
Normally B<EnciFS> spawns off as a daemon and runs in the background, returning
...
Normally B<EnciFS> provides a plaintext view of data on demand: it stores
...
For example, the following would create an encrypted view in /tmp/crypt-view.
    ...
    encifs --reverse /home/me /tmp/crypt-view

You could then copy the /tmp/crypt-view directory in order to have a copy of
the encrypted data.  You must also keep a copy of the file /home/me/.encifs6.xml
which contains the filesystem information.  Together, the two can be used to
reproduce the unencrypted data:

    ENCIFS6_CONFIG=/home/me/.encifs6.xml encifs /tmp/crypt-view /tmp/plain-view
...
B<EnciFS> takes everything returned from the program to be the password, except
for a trailing newline (\n) which will be removed.

For example, specifying B<--extpass>=I</usr/lib/ssh/ssh-askpass> will cause
B<EnciFS> to use ssh's password prompt program.  

B<Note>: B<EnciFS> reads at most 2k of data from the password program, and it
...
scripting encifs mounts.

Note that you should make sure the filesystem and mount points exist first.
Otherwise encifs will prompt for the filesystem creation options, which may
interfere with your script.

=item B<--anykey>

Turn off key validation checking.  This allows B<EnciFS> to be used with
secondary passwords.  This could be used to store a separate set of files in an
encrypted filesystem.  B<EnciFS> ignores files which do not decode properly, so
files created with separate passwords will only be visible when the filesystem
is mounted with their associated password.

Note that if the primary password is changed (using B<encifsctl>), the other
...
be enabled.  When mounting an existing filesystem, this causes encifs to exit
...
requested is one minute.  B<EnciFS> will not automatically unmount if there are
...
instead of exiting, B<EnciFS> stops allowing access to the filesystem by
...
Do not mount the filesystem when encifs starts; instead, delay mounting until
...
Attempt to make encifs behave as a typical multi-user filesystem.  By default,
...
provided by the filesystem.  Secondly, the B<--public> flag changes how encifs's
node creation functions work - as they will try and set ownership of new nodes
based on the caller identification.

B<Warning>: In order for this to work, encifs must be run as root -- otherwise
...
Setting this option makes EnciFS pass "attr_timeout=0" and "entry_timeout=0" to
FUSE. This makes sure that modifications to the backing file attributes that
occour outside EnciFS show up immediately in the EnciFS mount. The internal EnciFS
...
B<Encifs> adds the FUSE flags "use_ino" and "default_permissions" by default, as
...
The following command lines produce the same result:
    ...
    encifs raw crypt
    encifs --no-default-flags raw crypt -- -o use_ino,default_permissions

=item B<-o FUSE_ARG>

Pass through B<FUSE> args to the underlying library.  This makes it easy to
pass FUSE options when mounting B<EnciFS> via mount (and /etc/fstab).  Eg:

    mount encifs#/home/me-crypt /home/me -t fuse -o kernel_cache

Note that encifs arguments cannot be set this way.  If you need to set encifs
arguments, create a wrapper, such as  encifs-reverse;
...
Then mount using the script path
    ...
    mount encifs-reverse#/home/me /home/me-crypt -t fuse

=item B<-d>, B<--fuse-debug>

Enables debugging within the B<FUSE> library.  This should only be used if you
suspect a problem within B<FUSE> itself (not B<EnciFS>), as it generates a lot
...
of what is happening within B<EnciFS>.
...
The B<--> option tells B<EnciFS> to send any remaining arguments directly to
...
=item B<ENCIFS6_CONFIG>

Which config file (typically named .encifs6.xml) to use.
...
full output of encifs as it asks the user if they wish to create the filesystem:
...
special arguments to fusermount):
    ...
    % encifs ~/.crypt ~/crypt -- -n /dev/foo -c
...
reproducing the problem and send along the output with the problem report:
    ...
    % encifs -v -f ~/.crypt ~/crypt 2> encifs-report.txt
...
encrypted filenames.  You can use the I<encifsctl> program's I<decode> function
to decode filenames if desired.

=head1 CAVEATS

B<EnciFS> is not a true filesystem.  It does not deal with any of the actual
...
inherited by B<EnciFS> (or possibly be further limited).

One such limitation is filename length.  If your underlying filesystem limits
you to N characters in a filename, then B<EnciFS> will limit you to approximately
3*(N-2)/4.  For example if the host filesystem limits to 255 characters, then
B<EnciFS> will be limited to 189 character filenames.  This is because encrypted
filenames are always longer than plaintext filenames.

=head1 FILESYSTEM OPTIONS

When B<EnciFS> is given a root directory which does not contain an existing
B<EnciFS> filesystem, it will give the option to create one.  Note that options
...
Multiple instances of encifs can be run at the same time, including different
versions of encifs, as long as they are compatible with the current FUSE module
on your system.
...
As of version 1.5, B<EnciFS> now uses PBKDF2 as the default key derivation
...
If an B<EnciFS> filesystem configuration from 1.4.x is modified with version 1.5
(such as when using encifsctl to change the password), then the new PBKDF2
...
on what supported algorithms B<EnciFS> found in the encryption libraries.
...
This is the size (in bytes) that B<EnciFS> deals with at one time.  Each block
...
Having larger block sizes reduces the overhead of B<EnciFS> a little, but it can
...
B<New in 1.1>.  In previous versions of B<EnciFS>, each filename element in
...
changed.  If there are thousands of files, then EnciFS will have to do thousands
of renames.  It may also be possible that EnciFS will come across a file that it
...
B<New in 1.1>.  In previous versions of B<EnciFS>, each file was encoded in the
...
filename.  If an encrypted file is renamed outside of encifs, it will no longer
be decodable within encifs.  Note that unless Block MAC headers are enabled, the
...
B<EnciFS>.  B<EnciFS> will refuse to read data which does not pass the checksum,
...
When this is not enabled and if B<EnciFS> is asked to read modified or corrupted
data, it will have no way to verify that the decoded data is what was
originally encoded.

=item I<File-hole pass-through>

Make encifs leave holes in files.  If a block is read as all zeros, it will be
assumed to be a hole and will be left as 0's when read (not deciphered).  This
is required if accessing encifs using the SMB protocol.
...
The primary goal of B<EnciFS> is to protect data off-line.  That is, provide a
convenient way of storing files in a way that will frustrate any attempt to
read them if the files are later intercepted.

Some algorithms in B<EnciFS> are also meant to frustrate on-line attacks where
...
user's machine (and can therefore modify B<EnciFS>, or B<FUSE>, or the kernel
...
filesystem contents along with the algorithms B<EnciFS> supports to thwart them:
...
B<EnciFS> does not use any form of XOR encryption which would allow
...
B<EnciFS> for complete details.

=head1 AUTHORS

B<EnciFS> was written by B<< Valient Gough <vgough@pobox.com> >>.

Site : B<https://vgough.github.io/encifs/>.

Support, bug reports... : B<https://github.com/vgough/encifs>.

Mailing list : none.

Cygwin, Windows ports : B<https://github.com/vgough/encifs/wiki>.
